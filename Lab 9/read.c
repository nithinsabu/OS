...
//Added code
  ...
  {
	printf("MINIX 210010032: file write: %llu; nbytes = %zu; offset = %llu\n",vp->v_inode_nr, size, f->filp_pos);
  }
  if (rw_flag == READING && strcmp(vmp->m_mount_path, "/home") == 0)
  {
	printf("MINIX 210010032: file read: %llu; nbytes = %zu; offset = %llu\n",vp->v_inode_nr, size, f->filp_pos);
  }
